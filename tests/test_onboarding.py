...
class OnboardingTextTests(unittest.TestCase):
    def test_settings_use_basic_and_advanced_progressive_disclosure(self) -> None:
        ...
        self.assertIn("Preview Auto-Configure Fields", source)
        self.assertIn("Auto-Configure does not guess either language", source)
        self.assertIn("You still choose:", source)
        self.assertIn("Language for translations", source)
        self.assertIn('cards_form.addRow("Recognition templates"', source)
        self.assertIn('cards_form.addRow("Recall templates"', source)
        self.assertIn("recall cards show a contextual blank", source)
        ...
        self.assertIn("Recommended: 150,000 to 200,000 sentences. Coverage varies by deck and language.", source)
